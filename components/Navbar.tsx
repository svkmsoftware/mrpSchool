...
export default function Navbar() {
  ...
  return (
    <header className="w-full bg-white relative">
          ...
          <div className="flex items-center justify-between lg:justify-center h-16 md:h-20">
            ...
            {isScrolled && (
              <div className="lg:absolute left-2 xl:left-10 flex items-center animate-in fade-in slide-in-from-left-4">
                <img src="/images/mrpSchool_logo.png" className="h-10 w-auto" alt="Logo" />
              </div>
            )}
            ...
            <ul className="hidden lg:flex items-center gap-6 xl:gap-8">
              <li>
                <Link href="/" className="text-xs xl:text-sm font-bold text-slate-700 hover:text-primary uppercase tracking-wide">Home</Link>
              </li>
              {navigation.map((menu, index) => (
                ...
                >
                  <button className="flex items-center gap-1 text-xs xl:text-sm font-bold text-slate-700 hover:text-primary uppercase tracking-wide">
                    {menu.title}
                    <ChevronDown size={14} className={`transition-transform ${openIndex === index ? "rotate-180" : ""}`} />
                  </button>
                  {openIndex === index && (
                    <div className="absolute left-1/2 -translate-x-1/2 top-full w-56 bg-white shadow-2xl border border-slate-100 rounded-b-xl py-3 z-[110]">
                      {menu.items.map((item) => (
                        <Link key={item.name} href={item.href} className="block px-6 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-primary font-medium">
                          {item.name}
                        </Link>
                      ))}
                    </div>
      ...
      <div className={`fixed inset-0 z-[200] transition-visibility duration-300 ${isMobileMenuOpen ? "visible" : "invisible"}`}>
        {/* Backdrop */}
        <div 
        ...
        {/* Drawer */}
        <div className={`absolute right-0 top-0 h-full w-[85%] max-w-sm bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
          <div className="p-6 border-b flex justify-between items-center bg-slate-50">
            <div className="flex items-center gap-3">
                <img src="/images/mrpSchool_logo.png" className="h-8 w-auto" alt="Logo" />
                <span className="font-black text-slate-900 text-xs uppercase tracking-tighter">Menu</span>
            </div>
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-2">
            <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="block py-3 text-lg font-black text-slate-900 uppercase tracking-tighter border-b border-slate-100">Home</Link>
            {navigation.map((menu, index) => (
              <div key={menu.title} className="border-b border-slate-50">
                ...
                >
                  {menu.title}
                  <ChevronDown size={20} className={`transition-transform text-primary ${openIndex === index ? "rotate-180" : ""}`} />
                </button>
                <div className={`overflow-hidden transition-all duration-300 ${openIndex === index ? "max-h-[500px] pb-4" : "max-h-0"}`}>
